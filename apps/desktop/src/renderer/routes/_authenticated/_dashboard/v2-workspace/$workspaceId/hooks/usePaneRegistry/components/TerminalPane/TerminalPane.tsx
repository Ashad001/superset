		...
		terminalRuntimeRegistry.setLinkHandlers(
			terminalId,
			{
				...
				onFileUrlClick: (event, path) => {
					...
					event.preventDefault();
					// An image has no editor worth opening it in, so "external"
					// means the OS handler. Everything else goes through the shared
					// file path, read off the ref so this effect keeps stable deps.
					if (action === "external" && isImage) {
					...
					runFileLinkAction(linkActionDepsRef.current, { path }, action);
				},
				onLinkHover,
				onLinkLeave,
			},
