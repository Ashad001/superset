...
import type { OpenInExternalEditorOptions } from "renderer/routes/_authenticated/_dashboard/v2-workspace/$workspaceId/hooks/useOpenInExternalEditor";
import type {
	OpenFile,
	PaneViewerData,
...
export function runFileLinkAction(
	...
	// Fork: hand the file to the OS handler (Preview, Acrobat) rather than the
	// configured editor. Line/column are meaningless there.
	if (action === "system") {
		deps.openInExternalEditor(file.path, { defaultApp: true });
		return;
	}
	deps.onOpenFile(
		file.path,
